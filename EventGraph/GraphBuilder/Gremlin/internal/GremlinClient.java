package EventGraph.GraphBuilder.Gremlin.internal;
...
*/
	 ...
	 * AbstractGremlinGraphFactory creator) { return executeQueryAsync(query)
	 * .thenApply(result -> result .stream() .map(r -> creator.create(r))
	 * .collect(Collectors.toList())) .thenApply(Graph::createFrom); }
	 * 
	 * public CompletableFuture<Graph> explainQueryAsync(ScriptQuery query,
	 * AbstractGremlinGraphFactory creator) { RequestOptions.Builder builder =
	 * RequestOptions.build(); if (trimToNull(this.alias) != null) {
	 * builder.addAlias("g", this.alias); } query.getParams().forEach((k, v) ->
	 * builder.addParameter(k, v));
	 * 
	 * CompletableFuture<ResultSet> resultSet = withClient(client ->
	 * client.submitAsync(query.getExplainQuery(), builder.create()));
	 * 
	 * return resultSet .thenApply(result -> result .stream() .map(r ->
	 * creator.create(r)) .collect(Collectors.toList()))
	 ...
	 * RequestOptions.Builder builder = RequestOptions.build(); if
	 * (trimToNull(this.alias) != null) { builder.addAlias("g", this.alias); }
	 * theQuery.getParams().forEach((k, v) -> builder.addParameter(k, v)); return
	 * withClient(client -> client.submitAsync(theQuery.getQuery(),
	 * builder.create())); }
	 * 
	 * public <T> CompletableFuture<T>
	 * withRemoteTraversal(Function<GraphTraversalSource, CompletableFuture<T>>
	 * callersQuery) { ensureConnected(); DriverRemoteConnection connection =
	 * DriverRemoteConnection.using(this.cluster, this.alias); GraphTraversalSource
	 * traversalSource =
	 * AnonymousTraversalSource.traversal().withRemote(connection); return
	 * callersQuery.apply(traversalSource); }
	 */
