...
import EventGraph.graphStream.RepresentationLearning.mmdw.Learn.StoreAlphaWeight;
...
public class Linear {
    ...
    public static void crossValidation(Problem prob, Parameter param, int nr_fold, double[] target) {
        ...
        for (i = 0; i < nr_fold; i++) {
            ...
            StoreAlphaWeight ss=new StoreAlphaWeight();
            Model submodel = train(subprob, param,ss);
            for (j = begin; j < end; j++)
     ...
     */
    public static Model train(Problem prob, Parameter param, StoreAlphaWeight saws) {
        ...
        if (param.solverType.isSupportVectorRegression()) {
            ...
            if (param.solverType == SolverType.MCSVM_CS) {
               ...
               // for(int i=0;i<prob.l;i++)
                //	System.out.println(sub_prob.y[i]);

                SolverMCSVM_CS solver = new SolverMCSVM_CS(sub_prob, nr_class, weighted_C, param.eps);
                solver.solve(model.w);
                //********
                int nr_feature=StoreAlphaWeight.dimensionForSVM;
                saws.nr_class=nr_class;
                saws.weightB = new double[nr_class][nr_feature];//class*k
                saws.alphaB = new double[l][nr_class];//V_train*class
                System.err.println(nr_class);
                System.out.println("class_svm  "+nr_class+"   l_svm  "+l+"  feature  "+nr_feature);
                for(int i = 0; i < model.w.length; i ++){
                	saws.weightB[i%nr_class][i/nr_class] = model.w[i];
            	}
                double[] alpha = solver.getAlpha();
                for(int i = 0; i < alpha.length; i ++){
                	saws.alphaB[i/nr_class][i%nr_class] = alpha[i];
                }
                System.err.println("Read in Linear complete!!");
            } else {
                if (nr_class == 2) {
                    model.w = new double[w_size];
