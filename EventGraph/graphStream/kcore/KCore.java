...
public class KCore {
     ...
     */
    public static Pair<int[][], int[]> getDegeneracyCore(final int[][] graph, 
    		final int[] corenesses) {
        int maxK = 0;
     ...
     */
    public static Pair<int[][], int[]> getKCore(final int[][] graph,
    		final int[] corenesses, final int k) {
        ...
        for(int src = 0; src < graph.length; src++){
            if(corenesses[src] >= k){
                ...
                for(final int trg: graph[src]){
                    if(corenesses[trg] >= k) {
                        newGraph[indexToNewIndex.get(src)][count++] = 
                        		indexToNewIndex.get(trg);
                    }
                }
            }
        }
