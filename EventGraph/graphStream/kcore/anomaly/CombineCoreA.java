...
package EventGraph.graphStream.kcore.anomaly;
...
public class CombineCoreA {
    ...
    public static void main(String[] args) throws IOException {
        ...
        run(input,Import.load(input, ","), output, weight, false);
    };
     ...
     * @param weight weight that result of CoreA is multiplied by 
     * for being balanced with average degree
     */
    public static Set<Integer>  run(String input, int[][] graph, String output, int weight, 
    		boolean useTruss) throws IOException {

    	/**
    	 * idx
    	 */
    	 String mapperFile = input.concat(CoreDecomp.Label);
         
         Map<Integer,String> idx2Map = CoreDecomp.IDX2Map(mapperFile);
        ...
        writeResult(idx2Map,output, anomalies);
        return anomalies;
    }
     ...
     */
    public static void writeResult(Map<Integer, String> idx2Map, String output, Set<Integer> anomalies) throws IOException {
        final BufferedWriter bw = new BufferedWriter(new FileWriter(output));
        bw.write("vertex_index");
        bw.newLine();
        for(int node : anomalies) {
            bw.write(String.valueOf(node)+","+idx2Map.get(node));
            bw.newLine();
        }
        bw.close();
    }
     ...
     */
    public static int[][] run(int[][] graph, int[][] transpose, 
    		int numRows, int numCols, double[] rowSuspiciousness, 
    		double[] colSuspiciousness) {
