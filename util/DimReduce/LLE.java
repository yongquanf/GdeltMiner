...
import util.Util.smile.math.matrix.LU;
import util.Util.smile.math.matrix.EVD;
import util.Util.smile.math.matrix.Lanczos;
...
public class LLE {
    ...
    public LLE(double[][] data, int d, int k) {
        ...
        graph = new AdjacencyListGraph("alg");
        for (int i = 0; i < n; i++) {
       ...
       // int[][] cc = graph.bfs();
        ConnectedComponents cc0 = new ConnectedComponents();
		cc0.init(graph);
		cc0.compute();
		ConnectedComponent maxNodeComponent = cc0.getGiantComponent();
		Graph subGraph = GraphStreamUtil.subGraph(graph, maxNodeComponent.getNodeSet());
		
		
        int[] newIndex = new int[n];
        if (cc0.getConnectedComponentsCount() == 1) {
            index = new int[n];
            for (int i = 0; i < n; i++) {
                index[i] = i;
                newIndex[i] = i;
...
//        }
		...
		index = GraphStreamUtil.subGraphIndex(subGraph); 
		for (int i = 0; i < index.length; i++) {
          newIndex[index[i]] = i;
      }
        }
        ...
        EVD eigen = null;//im.eigen(Math.min(10*(d + 1), n - 1));
