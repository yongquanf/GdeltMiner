...
package util.Util.smile.math.matrix;
...
public interface DenseMatrix extends Matrix, MatrixMultiplication<DenseMatrix, DenseMatrix> {
    ...
    default EVD eigen(int k) {
        return eigen0(k, 1.0E-8, 10 * nrows());
    }
     ...
     */
    default EVD eigen0(int k, double kappa, int maxIter) {
     ...
     * @return
     */
    default DenseMatrix times(DenseMatrix b) {
    	 if (ncols() != b.nrows() ) {
             throw new IllegalArgumentException("Matrix is not of same size.");
         }
    	 
    	 int outRows = this.nrows();
    	 int outColumns = b.ncols();
    	 
    	 DenseMatrix C = Matrix.newInstance(outRows, outColumns, 0);
    	 //temp
    	 double[] Bcolj = new double[b.nrows()];
     ...
     * @return sqrt(A)
     */
	 default DenseMatrix sqrt() {
		 DenseMatrix C = Matrix.newInstance(nrows(), ncols(), 0);
		 for(int i=0;i<C.nrows();i++) {
			 for(int j=0;j<C.ncols();j++) {
				 C.set(i, j, Math.sqr(get(i, j)));
			 }
		 }
		 return C;
	 }
	  ...
	  * @return
	  */
	 default DenseMatrix  tan2(DenseMatrix y) {
		 
		 DenseMatrix C = Matrix.newInstance(nrows(), ncols(), 0);
		 for(int i=0;i<C.nrows();i++) {
			 for(int j=0;j<C.ncols();j++) {
				 double val = Math.atan2(get(i, j), y.get(i, j));
				 C.set(i, j, val);
			 }
		 }
		 return C;
	 ...
	 /**
		 ...
		 * @param d
		 */
		default void mod(double d) {
			
			//DenseMatrix C = Matrix.newInstance(nrows(), ncols(), 0);
			for(int i=0;i<nrows();i++) {
				 for(int j=0;j<ncols();j++) {
					 double val = get(i, j)%d;
					  set(i, j, val);
				 }
			 }
		...
		default double[] getRowVector(int rowIndex,int rowLen) {
			double[] out = new double[rowLen];
			for(int j=0;j<rowLen;j++) {
				out[j]=get(rowIndex, j);
			}
			return out;
	}
}
