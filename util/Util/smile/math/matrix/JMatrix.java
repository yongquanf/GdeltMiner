...
import util.Util.smile.math.Complex;
import util.Util.smile.math.Math;

/**
 * A pure Java implementation of DenseMatrix whose 
 * data is stored in a single 1D array of
...
public class JMatrix implements DenseMatrix {
    ...
    public double get(int i, int j) {
    	int pos = j*nrows + i;
    	if(pos>=A.length) {
    		logger.error("get overflow: "+j+", rows: "+nrows + i+", "+pos+", but"+ A.length);
    		return  A[pos];
    	}{
    		return A[pos];
    	}
    ...
    public double set(int i, int j, double x) {
    	...
    	int pos =j*nrows + i;
    	if(pos>=A.length) {
    		logger.error("set overflow: "+j+", rows: "+nrows + i+", "+pos+", but"+ A.length);
    		return  A[pos] = x;
    	}else {
    			return A[pos] = x;
    		}
