...
import util.Util.smile.stat.distribution.GaussianDistribution;
...
public interface Matrix extends Serializable {
     ...
     */
    default double[] diag() {
        int n = util.Util.smile.math.Math.min(nrows(), ncols());
     ...
     * @return
     */
	static Matrix newInstance(int r, int c, int[][] adjMat) {
		// TODO Auto-generated method stub
		DenseMatrix out = newInstance(r, c, 0);
		for(int i=0;i<adjMat.length;i++) {
			int[]vec = adjMat[i];
			for(int j=0;j<vec.length;j++) {
				int index = vec[j];
				out.set(i, index, vec[j]);
			}
		}//end
		return out;
	}
